...
void ValveType::set_name(const std::string& name) {
  name_ = name;
}
...
void ValveType::next_valve_size() {
  size_index_ = size_index_ + 1 >= sizes_.size() ? 0 : size_index_ + 1;
}

void ValveType::previous_valve_size() {
  size_index_ = size_index_ <= 0 ? sizes_.size() - 1 : size_index_ - 1;
}
...
float FlowMeter::get_setup_priority() const {
...
void FlowMeter::add_valve_type(ValveType* valve_type) {
  valve_types_.push_back(valve_type);
}
...
const std::string& FlowMeter::active_valve_type_name() {
  return active_valve_type().get_name();
}
...
const std::string& FlowMeter::active_valve_size() const {
  return active_valve().get_size();
}

void FlowMeter::next_valve_size() {
  active_valve_type().next_valve_size();
}

void FlowMeter::previous_valve_size() {
  active_valve_type().previous_valve_size();
}

float FlowMeter::active_valve_setting() const {
  return active_valve().get_setting();
}

bool FlowMeter::next_valve_setting() {
  return active_valve().next_valve_setting();
}

bool FlowMeter::previous_valve_setting() {
  return active_valve().previous_valve_setting();
}
